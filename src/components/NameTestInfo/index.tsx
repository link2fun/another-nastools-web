  ...
  return (
    <div className={'flex flex-wrap '}>
      {mediaInfo && !!mediaInfo.type && (
        <span title={'类型'} className={'ml-2 mb-1 badge-outline text-blue-500 border-blue-500'}>
          {mediaInfo.type}
        </span>
      )}
      {mediaInfo && !!mediaInfo.category && (
        <span title={'类别'} className={`ml-2 mb-1 badge-outline text-blue-500 border-blue-500`}>
          {mediaInfo.category}
        </span>
      )}
      {mediaInfo && !!mediaInfo.name && (
        <span title={'识别名称'} className={'ml-2 mb-1 badge-outline text-red-500 border-red-500'}>
          <a
      ...
      {mediaInfo && !!mediaInfo.title && (
        <span title={'标题'} className={'ml-2 mb-1 badge-outline text-red-500 border-red-500'}>
          {mediaInfo.title}
        </span>
      )}
      {mediaInfo && !!mediaInfo.year && (
        <span
          title={'年份'}
          className={'ml-2 mb-1 badge-outline text-orange-500 border-orange-500'}
        >
          {mediaInfo.year}
        </span>
      )}
      ...
      {mediaInfo && !!mediaInfo.tmdbid && mediaInfo.tmdbid !== 0 && (
        <span
          title={'TMDB ID'}
          className={'ml-2 mb-1 badge-outline text-green-500 border-green-500'}
        >
          <a
      ...
      {mediaInfo && !!mediaInfo.restype && (
        <span title={'质量'} className={'ml-2 mb-1 badge-outline'}>
          {mediaInfo.restype}
        </span>
      )}
      {mediaInfo && !!mediaInfo.pix && (
        <span title={'分辨率'} className={'ml-2 mb-1 badge-outline'}>
          {mediaInfo.pix}
        </span>
      )}
      {mediaInfo && !!mediaInfo.video_codec && (
        <span title={'视频编码'} className={'ml-2 mb-1 badge-outline'}>
          {mediaInfo.video_codec}
        </span>
      )}
      {mediaInfo && !!mediaInfo.audio_codec && (
        <span title={'音频编码'} className={'ml-2 mb-1 badge-outline'}>
      ...
      {mediaInfo && !!mediaInfo.team && (
        <span
          title={'制作组/字幕组'}
          className={'ml-2 mb-1 badge-outline text-yellow-500 border-yellow-500'}
        >
          {mediaInfo.team}
        </span>
      )}
